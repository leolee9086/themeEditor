  ...
  <div class="fn__flex fn__flex-column fn__flex-1" v-if="background.type !== 'multi'">
    ...
    <div class="fn__flex fn__flex-1">
      <div class="fn__flex fn__flex-column">
        ...
        <input v-if="background.type === 'gradient'" type="text" v-model="background.image" placeholder="gradient"
          title="Enter the gradient">
      ...
      <div class="fn__flex fn__flex-column">
        ...
        <select v-if="background.type !== 'color'" v-model="background.attachment">
          <option value="scroll">Scroll</option>
          <option value="fixed">Fixed</option>
          <option value="local">Local</option>
        </select>
        <select v-if="background.type !== 'color'" v-model="background.origin">
          <option value="padding-box">Padding Box</option>
          <option value="border-box">Border Box</option>
          <option value="content-box">Content Box</option>
        </select>
        <select v-if="background.type !== 'color'" v-model="background.clip">
          <option value="border-box">Border Box</option>
          <option value="padding-box">Padding Box</option>
          <option value="content-box">Content Box</option>
...
export default {
  components: { BackgroundPositionInput },
  props: ['background', 'index', 'backgrounds'],
  watch: {
    'background.repeatX': function (newVal, oldVal) {
      this.background.repeat = `${this.background.repeatX} ${this.background.repeatY}`;
    },
    'background.repeatY': function (newVal, oldVal) {
      this.background.repeat = `${this.background.repeatX} ${this.background.repeatY}`;
    },
  },
}
